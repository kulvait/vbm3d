...
//
#define clo_usage(usage) cmd_opt::option((char*)0, argc, argv, (char*)0, usage)
#define clo_help(str) cmd_opt::option((char*)0, argc, argv, str, (char*)0)
#define clo_option(name, defaut, usage) cmd_opt::option(name, argc, argv, defaut, usage)
#define clo_argument(pos) cmd_opt::argument(pos, argc, argv)
#define clo_argument1(pos, s0) cmd_opt::argument(pos, argc, argv, 1, s0)
#define clo_argument2(pos, s0, s1) cmd_opt::argument(pos, argc, argv, 2, s0, s1)
#define clo_argument3(pos, s0, s1, s2) cmd_opt::argument(pos, argc, argv, 3, s0, s1, s2)
#define clo_argument4(pos, s0, s1, s2, s3) cmd_opt::argument(pos, argc, argv, 4, s0, s1, s2, s3)
...
{
    return ("this is not windows") ? (s ? s + 1 + strfind(s, '/') : 0)
                                   : (s ? s + 1 + strfind(s, '\\') : 0);
}

inline const char* option(const char* const name,
                          const int argc,
                          char** argv,
                          const char* defaut,
                          const char* const usage = 0)
{
    ...
    if(name)
    {
        if(argc > 0)
        {
            ...
            res = defaut;
        if(visu && usage)
            fprintf(stderr, "    %s%-8s%s = %-12s : %s%s%s\n", t_bold, name, t_normal,
                    res ? res : "0", t_purple, usage, t_normal);
    }
    return res;
}

inline bool option(const char* const name,
                   const int argc,
                   char** argv,
                   const bool defaut,
                   const char* const usage = 0)
{
    ...
    const bool res
        = s ? (strcasecmp(s, "false") && strcasecmp(s, "off") && strcasecmp(s, "0")) : defaut;
    option(name, 0, 0, res ? "true" : "false", usage);
    return res;
}

inline int option(const char* const name,
                  const int argc,
                  char** argv,
                  const int defaut,
                  const char* const usage = 0)
...
inline char option(const char* const name,
                   const int argc,
                   char** argv,
                   const char defaut,
                   const char* const usage = 0)
...
inline float option(const char* const name,
                    const int argc,
                    char** argv,
                    const float defaut,
                    const char* const usage = 0)
...
inline double option(const char* const name,
                     const int argc,
                     char** argv,
                     const double defaut,
                     const char* const usage = 0)
...
inline const char*
argument(const unsigned int nb, const int argc, char** argv, const unsigned int nb_singles = 0, ...)
{
    for(int k = 1, pos = 0; k < argc;)
    {
        ...
        if(option)
        {
            ...
            for(unsigned int i = 0; i < nb_singles; ++i)
                if(!strcasecmp(item, va_arg(ap, char*)))
                {
                    single_option = true;
                    break;
                }
