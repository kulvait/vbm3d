                             ...
                             uint64_t i_frameStep)
{
    ...
    //! open first frame and allocate memor
    uint64_t loadedElementsCount = 0;
    uint64_t loadedFramesCount = 0;
    std::vector<float>::iterator p_data;
    {
        ...
        //        printf("frame.size() = %lu data.size() = %lu, sz.whcf=%lu loadedElementsCount=%lu
        //        frame=%lu\n",
        //               frame.size(), data.size(), sz.whcf, loadedElementsCount,
        //               loadedElementsCount/sz.wh);
        p_data = std::copy(frame.begin(), frame.end(), data.begin());
        loadedElementsCount += frame.size();
        loadedFramesCount++;
    }

    //! load rest of frames
    for(uint64_t f = i_firstFrame + i_frameStep; f <= i_lastFrame; f += i_frameStep)
    {
        char filename[1024];
        ...
        if(frameSize.width != sz.width || frameSize.height != sz.height
           || frameSize.nChannels != sz.channels)
            throw std::runtime_error("Video<T>::loadVideo: size of " + std::string(filename)
                                     + " does not match video size");
        ...
        //               frame.size(), data.size(), sz.whcf, loadedElementsCount,
        //               loadedElementsCount/sz.wh);
        if(loadedFramesCount % 500 == 0)
            printf("loadedFramesCount = %05lu/%05lu\n", loadedFramesCount, sz.frames);
        loadedElementsCount += frame.size();
        loadedFramesCount++;
    }
    printf("loadedFramesCount = %05lu/%05lu\n", loadedFramesCount, sz.frames);
...
template <>
void Video<float>::loadFullFlow(const std::string i_pathToFiles,
                                uint64_t i_firstFrame,
                                uint64_t i_lastFrame,
                                uint64_t i_frameStep)
{
    ...
    std::vector<float>::iterator p_data;
    {
        ...
        if(loadFlow(filename, frame, frameSize) == EXIT_FAILURE)
            throw std::runtime_error("Video<T>::loadFlow: loading of " + std::string(filename)
                                     + " failed");
    ...
    for(uint64_t f = i_firstFrame + i_frameStep; f <= i_lastFrame; f += i_frameStep)
    {
        ...
        if(frameSize.width != sz.width || frameSize.height != sz.height
           || frameSize.nChannels != sz.channels)
            throw std::runtime_error("Video<T>::loadFlow: size of " + std::string(filename)
                                     + " does not match video size");
                             ...
                             float i_pmax) const
{
    ...
    uint64_t lastFrame = i_firstFrame + sz.frames * i_frameStep;
    std::vector<float> frame(sz.whc);
    std::vector<float>::const_iterator p_data = data.begin();
    for(uint64_t f = i_firstFrame; f < lastFrame; f += i_frameStep, p_data += sz.whc)
    {
        ...
        if(saveImage(filename, frame, frameSize, i_pmin, i_pmax) == EXIT_FAILURE)
            throw std::runtime_error("Video<T>::saveVideo: writing of " + std::string(filename)
                                     + " failed");
    }
                                  ...
                                  uint64_t i_frameStep) const
{
    ...
    for(size_t c = 0; c < C; ++c)
        ...
        {
            if(C > 1)
                std::sprintf(channel, "_ch%d", (int)c);
            else
                channel[0] = 0;
            if(F > 1)
                std::sprintf(frame, "_%03d", (int)f);
            else
                frame[0] = 0;
...
template <>
std::vector<float> Video<float>::dct_shuffle_video(int initFrame, int finalFrame)
{
...
void Video<float>::transformVideoFromBayer(const Video<float>& input)
{
    ...
    data.resize(sz.whcf);
    for(uint64_t x = 0; x < sz.width; ++x)
        for(uint64_t y = 0; y < sz.height; ++y)
            for(uint64_t t = 0; t < sz.frames; ++t)
            {
...
void Video<float>::transformVideoToBayer(const Video<float>& input)
{
    ...
    data.resize(sz.whcf);
    for(uint64_t x = 0; x < input.sz.width; ++x)
        for(uint64_t y = 0; y < input.sz.height; ++y)
            for(uint64_t t = 0; t < input.sz.frames; ++t)
            {
